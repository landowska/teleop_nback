...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\NIRS-PC\\Desktop\\teleoperation_experiment_v1\\teleopartion_nback_lastrun.py',
...
for thisBlock in blocks:
    ...
    # set up handler to look after randomisation of conditions etc
    oneback_meta = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
        seed=None, name='oneback_meta')
    thisExp.addLoop(oneback_meta)  # add the loop to the experiment
    thisOneback_meta = oneback_meta.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisOneback_meta.rgb)
    if thisOneback_meta != None:
        for paramName in thisOneback_meta:
            exec('{} = thisOneback_meta[paramName]'.format(paramName))
    
    for thisOneback_meta in oneback_meta:
        currentLoop = oneback_meta
        # abbreviate parameter names if possible (e.g. rgb = thisOneback_meta.rgb)
        if thisOneback_meta != None:
            for paramName in thisOneback_meta:
                exec('{} = thisOneback_meta[paramName]'.format(paramName))
    ...
    # completed 1.0 repeats of 'oneback_meta'
    
    # get names of stimulus parameters
    if oneback_meta.trialList in ([], [None], None):
        params = []
    else:
        params = oneback_meta.trialList[0].keys()
    # save data for this loop
    oneback_meta.saveAsExcel(filename + '.xlsx', sheetName='oneback_meta',
        stimOut=params,
        dataOut=['n','all_mean','all_std', 'all_raw'])
    oneback_meta.saveAsText(filename + 'oneback_meta.csv', delim=',',
        stimOut=params,
        dataOut=['n','all_mean','all_std', 'all_raw'])
    ...
    # set up handler to look after randomisation of conditions etc
    threebloc_kmeta = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
        seed=None, name='threebloc_kmeta')
    thisExp.addLoop(threebloc_kmeta)  # add the loop to the experiment
    thisThreebloc_kmeta = threebloc_kmeta.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisThreebloc_kmeta.rgb)
    if thisThreebloc_kmeta != None:
        for paramName in thisThreebloc_kmeta:
            exec('{} = thisThreebloc_kmeta[paramName]'.format(paramName))
    
    for thisThreebloc_kmeta in threebloc_kmeta:
        currentLoop = threebloc_kmeta
        # abbreviate parameter names if possible (e.g. rgb = thisThreebloc_kmeta.rgb)
        if thisThreebloc_kmeta != None:
            for paramName in thisThreebloc_kmeta:
                exec('{} = thisThreebloc_kmeta[paramName]'.format(paramName))
    ...
    # completed 1.0 repeats of 'threebloc_kmeta'
    
    # get names of stimulus parameters
    if threebloc_kmeta.trialList in ([], [None], None):
        params = []
    else:
        params = threebloc_kmeta.trialList[0].keys()
    # save data for this loop
    threebloc_kmeta.saveAsExcel(filename + '.xlsx', sheetName='threebloc_kmeta',
        stimOut=params,
        dataOut=['n','all_mean','all_std', 'all_raw'])
    threebloc_kmeta.saveAsText(filename + 'threebloc_kmeta.csv', delim=',',
        stimOut=params,
        dataOut=['n','all_mean','all_std', 'all_raw'])
